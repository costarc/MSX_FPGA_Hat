...
INIT:
        ...
        ; ------------------------------------------------------------------
        ; INTERRUPTS ENABLED (2026-08-18) - this is the whole point of this
        ; variant, and it closes the last untested path.
        ;
        ; Every mapper test so far has run with DI. That was deliberate (the
        ; ISR must not be counted as a mapper error), but it means we have
        ; NEVER tested the mapper in the regime Nextor actually runs in.
        ;
        ; With interrupts on, the 60Hz handler performs inter-slot calls, and
        ; every one of those WRITES 0xFFFF - the subslot register. That is the
        ; one path still unmeasured, and the one this project has patched most
        ; heavily (stability voting, minimum window length, always-commit
        ; fallback). If exp_slot mis-captures even one of those writes, page 2
        ; momentarily re-routes to the ROM subslot and reads stop coming from
        ; our RAM at all.
        ;
        ; That would explain everything still open: DI soak clean over ~27MB
        ; (no FFFF writes at all), ports clean, SD working, yet Nextor - which
        ; runs with interrupts on and writes FFFF constantly - hangs right
        ; after NEXTOR.SYS loads.
        ;
        ; The BIOS 60Hz handler uses page 0 (BIOS) and page 3 (workspace); it
        ; does not write page 2, so it should not produce false mismatches. If
        ; errors DO appear, look at the g= value: ROM-like or another
        ; segment's pattern means routing changed underneath us.
        ; ------------------------------------------------------------------
        ei
...
; ---------------------------------------------------------------- strings
MSG_BANNER:  db "SDMapper V2.1b SOAK (EI)",13,10
             db "Interrupts ON - tests FFFF path",13,10,0
