...
## ---------------------------------------------------------------------------
## D, RD_n, WR_n, MREQ_n, IORQ_n, SLTSL_n, M1_n and RESET_n are all genuinely
## asynchronous to the clocks driving this design - that is exactly why every
...
##
## NOTE: A[*] and CS_n are intentionally NOT listed here. The old 16-bit "A"
## port no longer exists (replaced by the 8-bit, time-multiplexed A_MUX, which
## is captured deterministically by the address-capture state machine rather
## than through a generic "_meta" synchronizer - see the "ADDRESS BUS
## MULTIPLEXING" note in SDMapper_Top.vhd). CS_n is unused by this design and
## gets optimized away by synthesis, so it has no matching port at this stage
## either - both previously produced "Ignored filter ... could not be matched
## with a port" warnings, which is expected and harmless.
##
...
## and the capturing clock to check against - "unconstrained" is not the same
## as "intentionally asynchronous" as far as the tool is concerned, so without
...
## ffff_meta.
set_false_path -from [get_ports {D[*] RD_n WR_n MREQ_n IORQ_n SLTSL_n M1_n RESET_n}] -to [get_registers {*_meta}]
