-- tb_spi2.vhd
--
-- GHDL-only functional testbench for spi2.vhd IN ISOLATION (not through the
...
architecture sim of tb_spi2 is
...
begin
	...
	begin
		...
		-- Widened for the SPI_DIV_MAX clock divider (2026-08-12): a full
		-- 8-bit transfer now takes ~8x longer (SCLK ~= clock_i/8 instead of
		-- clock_i/2), plus the fixed 15-cycle post-transfer wait_cnt_q
		-- countdown - 60 cycles is no longer enough headroom.
		wait until wait_n_o = '1' for 160 * CLK_PERIOD;
